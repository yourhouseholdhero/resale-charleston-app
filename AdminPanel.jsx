    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="flex justify-center mb-6 gap-2 flex-wrap">
        <button
          ...
          className={`px-4 py-2 rounded-md ${selectedTab === 'Inventory' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          Inventory
        </button>
        <button
          onClick={() => setSelectedTab('AddItem')}
          className={`px-4 py-2 rounded-md ${selectedTab === 'AddItem' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          Add Item
        </button>
        <button
          onClick={() => setSelectedTab('OwnerProfiles')}
          className={`px-4 py-2 rounded-md ${selectedTab === 'OwnerProfiles' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          Owner Profiles
        </button>
      </div>
